"""
    TonyHawkSaveSign.py - Python script to reclaculate checksums in Tony Hawk Pro Skater save files.
    Author: Grimdoomer
"""
...
"""
    Park file layout:
    0x00    XboxSaveHash        // Content signature generated with XCalSig
    0x14    HeaderChecksum      // Checksum of the header data
    0x18    HeaderDataSize      // Size of the header data to be checksumed
    0x1C    DataSize            // Size of the main data portion of the save to be checksumed
    0x10    FileType?           // Type of file, ex park, save, etc?
    0x14    DataChecksum        // Checksum of the main data portion of the save file
"""
...
def verifyParkFile(file) -> bool:
    ...
    # Parse the header.
    startpos = 20
    headerChecksum = int.from_bytes(saveData[startpos+0:startpos+4], 'little')
    headerDataSize = int.from_bytes(saveData[startpos+4:startpos+8], 'little')
    dataSize = int.from_bytes(saveData[startpos+8:startpos+12], 'little')
    ...
    headerChecksumNew = computeCRC(saveData[startpos+16:], headerDataSize)
...
def signParkFile(file) -> bool:
    ...
    saveData[startpos+0:startpos+4] = int.to_bytes(headerChecksumNew, 4, 'little')
