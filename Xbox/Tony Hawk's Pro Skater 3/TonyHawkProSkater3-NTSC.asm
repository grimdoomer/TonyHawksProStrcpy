...
; unlike THPS4 we don't have the 136 bytes of header data on the data segment to use for a memcpy stub. Instead we
; use a ROP chain to copy the main shell code payload to an executable region of memory. All of the save data is stored
; on the heap so when we do the overflow we are overwriting heap data. The game uses a custom memory allocator for the heap, and each
; allocation is wrapped with a header that contains a pointer to a vtable that's used when the allocation is free'd.
; We use the overflow to overwrite the vtable pointer in the next heap allocation following the gap name, which when
; free'd will execute a ROP chain that performs a stack pivot, copies the shell code into the data segment, and finally
; jumps to the shell code.
;
