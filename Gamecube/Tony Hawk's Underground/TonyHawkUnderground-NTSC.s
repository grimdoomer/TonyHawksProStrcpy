# Description: Save game exploit for Tony Hawk's Underground for Gamecube (NTSC)
# Author: Grimdoomer (THUG modifications by Cubified)

.set GapDataStartFileOffset,            0x3a70
.set SaveBufferMemoryAddress,           0x80a05cf2      # Points to start of save file in memory (accounts for 64 byte header in .gci file)

# Game function and data addresses:
.set __OSStopAudioSystem,               0x802072e0
.set CARDOpen,                          0x8022250c
.set CARDRead,                          0x80222ea8
.set CARDClose,                         0x80222684

.set FlushICache,                       0x80207484      # ICInvalidateRange (ICFlushRange is absent from binary)
.set FlushDCache,                       0x802073f8
.set memcpy,                            0x8020f1b8
